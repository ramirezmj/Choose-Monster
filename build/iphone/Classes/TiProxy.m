...
//Rare exceptions to indicate a bug in the _choosemonster code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_choosemonster = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_choosemonster]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.choosemonster.module"
